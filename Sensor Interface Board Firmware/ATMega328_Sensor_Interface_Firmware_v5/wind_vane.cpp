...
wind_vane_array_ wind_vane_data;

void wind_vane_array_::begin(bool _debug)
{
  // This sets up the unit
  // Sort out wind vane array
  read_direction_array();  // Gets data from EEPROM
  find_lower_bands(_debug);
  find_upper_bands(_debug);
  wind_vane_data.find_rollover_index(_debug);
}


void wind_vane_array_::find_lower_bands(bool _debug)
{
...
void wind_vane_array_::find_upper_bands(bool _debug)
{
...
void wind_vane_array_::find_rollover_index(bool _debug)
{
...
void wind_vane_array_::build_direction_array(uint16_t _vane_value)
{
...
void wind_vane_array_::read_direction_array()
{
  // Gets the direction data stored in EEPROM these are ints, not bytes, hence need to create longer data...
  for (int y = 0; y < 8; y++)
  {
    int address = (y * 2) + EEPROM_VANE_ADD;
    vane_bands[y] = word(EEPROM.read(address), EEPROM.read(address + 1));
  }
}

void wind_vane_array_::write_direction_array(int _direction, uint16_t _value)
{
  //Writes the data to the specified direction
  byte byte1 = _value >> 8;
  byte byte2 = _value & 0xFF;
  int address = (_direction * 2) + EEPROM_VANE_ADD;
  EEPROM.write(address, byte1);
  EEPROM.write(address + 1, byte2);
}

void wind_vane_array_::reset_vane_direction_array()
{
  for (int y = 0; y < 8; y++)
  {
...
String wind_vane_array_::return_direction(uint16_t _vane_value)
{
