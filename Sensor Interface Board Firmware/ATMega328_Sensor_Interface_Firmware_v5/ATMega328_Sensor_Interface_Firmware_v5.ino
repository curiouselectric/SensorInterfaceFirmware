...
float getSensorData(int _channel_number) {
  ...
  switch (_channel_number) {
    case 0:
      if (myPzem003Sensor.readVoltage(SENSOR_ID) == true) {
        _data = myPzem003Sensor.dcVoltage;
      } else {
#ifdef DEBUG_PZEM003
        Serial.println(F("PZEM ERR"));
        _data = 0.0;
#endif
      }
      break;
    ...
    case 1:
      if (myPzem003Sensor.readCurrent(SENSOR_ID) == true) {
        _data = myPzem003Sensor.dcCurrent;
      } else {
#ifdef DEBUG_PZEM003
        Serial.println(F("PZEM ERR"));
        _data = 0.0;
    ...
    case 2:
      if (myPzem003Sensor.readPower(SENSOR_ID) == true) {
        _data = myPzem003Sensor.dcPower;
      } else {
#ifdef DEBUG_PZEM003
        Serial.println(F("PZEM ERR"));
        _data = 0.0;
    ...
    case 3:
      if (myPzem003Sensor.readEnergy(SENSOR_ID) == true) {
        _data = myPzem003Sensor.dcEnergy;
      } else {
#ifdef DEBUG_PZEM003
        Serial.println(F("PZEM ERR"));
        _data = 0.0;
...
// Initialise for the different sensors:
#ifdef PZEM_003_RS485_SENSOR
  my_sensor_data[0].data_send_time = DATA_READ_S;  // 10 second reads
  my_sensor_data[1].data_send_time = DATA_READ_S;
  my_sensor_data[2].data_send_time = DATA_READ_S;
  my_sensor_data[3].data_send_time = DATA_READ_S;
